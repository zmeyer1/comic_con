...
class OpenAIBase:
    def __init__(self):
        ...
        """
        ## Pull openai_key and set it globally for the old API
        self.key = os.environ.get("OPENAI_API_KEY")
        if not self.key:
            raise ValueError("OPENAI_API_KEY environment variable not set")
        openai.api_key = self.key  # Set the API key globally for old API
...
class VisionToText(OpenAIBase):
    ...
    def viz_to_text(self, img='default', bbox=[0, 0, 640, 480], prompt_filename=None, prompt = "what object do you see?"
,
 max_length=1000):
        '''
    ...
    def chat(self, message, system_prompt="You are a helpful AI assistant specializing in comics, anime, and cosplay."):
        """
        Simple text chat with OpenAI using old APIcd
        """
        try:
            response = openai.ChatCompletion.create(  # Changed from self.client.chatCompletion.create
                model="gpt-4o",
                messages=[
...
class TextToText(OpenAIBase):
    ...
    def text_to_text(self, system_filename=None, system_prompt='Hello!'):
        ...
        ## Create the chat completion request using the OpenAI API (old syntax)
        try:
            response = openai.ChatCompletion.create(  # Changed from self.client.chat.completions.create
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": system_prompt},
